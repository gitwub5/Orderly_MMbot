...
export async function spreadOrder(client: MainClient, config: StrategyConfig, logger: winston.Logger) {
    const { symbol, orderQuantity, orderLevels, orderSpacing, tradePeriodMs, stdDevPeriod, gamma, k } = config;
        ...
        if (prediction === 1) {
            ...
            for (let level = 0; level < orderLevels; level++) {
                await client.placeOrder(symbol, 'BID', 'BUY', null, orderQuantity, {
                    body: JSON.stringify({ level: level,  visible_quantity: 0 })
                });
            }
            ...
            for (let level = 0; level < orderLevels; level++) {
                ...
                let buyPrice = fixPrecision(midPrice - buyPriceOffset, config.precision);
                let sellPrice = fixPrecision(midPrice + sellPriceOffset, config.precision);
            ...
            await delay(2000);  // Add await to ensure delay is handled properly
        }
    }
